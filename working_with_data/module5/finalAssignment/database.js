...
export async function createTable(){
  ...
  console.log("Creating the SQL table");
  db.transaction(
    (tx) => {
      tx.executeSql(
        'create table if not exists menuitems (uuid integer primary key not null, title text, price text, category text);'
      );
    })
...
export function saveMenuItems(menuItems) {
  console.log("The menuItems that we will put into the database:");
  console.log(menuItems);

  const query = `insert into menuitems (uuid, title, price, category) values ${menuItems
    .map((item) => `(${item.id}, '${item.title}', '${item.price}', '${item.category}')`)
    	.join(', ')};`;
  console.log("The query to insert the fetched data into the database:");
  console.log(query);

  console.log("populating the SQL table with data");
  db.transaction((tx) => {
...
export async function filterByQueryAndCategories(query, activeCategories) {
  const cat_string = activeCategories.map((cat_i) => `'${cat_i}'`).join(', ')
  console.log("cat_string: ")
  console.log(cat_string)
  let filter_query = `select * from menuitems where category in (${cat_string})`;
  if (query != "") {
    filter_query = filter_query.concat(` and title like '%${query}%';`);}
  else {
    filter_query = filter_query.concat(`;`);}

  console.log("The query to filter by:")
  console.log(filter_query)

  return new Promise((resolve) => {
    db.transaction((tx) => {
      tx.executeSql(filter_query, [], (_, { rows }) => {
        resolve(rows._array);
      });
    });
  });
