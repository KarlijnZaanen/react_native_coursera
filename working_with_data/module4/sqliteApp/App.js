...
export default function App() {
  ...
  // this function gets ran when the user clicks on the edit icon, and opens a pop-up with a dialogue incouding a field to enter the updated name
  ...
  // this function gets ran when the user is done updating the customer name, when the user dismisses the update-dialogue.
  const hideDialog = (updatedCustomer) => {
    ...
    const newCustomers = customers.map((customer) => { 
     // go over all the customers in the customers list, only if the customer.uid == updatedCustomer.uid, then insert the updatedCustomer. Else, simply keep whatever was already in the list.
   ...
   // now actually set the local state customers as equal to the newCustomers list that we just created
   setCustomers(newCustomers);    
 
   // 2. Create a SQL transaction to edit a customer. Make sure if two names are the same, only the selected item is edited
   db.transaction((tx) => { 
     tx.executeSql( 
  ...
  // this function gets ran when the user clicks on the delete icon
  const deleteCustomer = async (customer) => {
    ...
    console.log("The old list of customers:")
    console.log(customers)
    // 1. Set the new local customer state
    const newCustomers = customers.filter((c) => c.uid !== customer.uid); 
    console.log("The new list, after deleting a customer:")
